...
export default function PlanForm({ row, allDoctors, blogAccounts, onSaved, onDeleted }: Props) {
  const [form, setForm] = useState(EMPTY_FORM);
  const [saving, setSaving] = useState(false);
  const [suggestions, setSuggestions] = useState<string[]>([]);
  const [activeDoctorIdx, setActiveDoctorIdx] = useState<number | null>(null);

  const { handleSave, handleSaveAndEdit, handleDelete } = usePlanForm(row, allDoctors, onSaved);
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
        ...
        <div className="max-w-2xl mx-auto space-y-5">
          ...
          <Section title="기본 정보">
            <div className="grid grid-cols-2 gap-3">
              <Field label="마케터">
                <Input value={form.marketer} onChange={v => setField('marketer', v)} placeholder="홍길동" />
              </Field>
              <Field label="날짜">
                <input
                  type="date"
                  className="w-full text-sm text-gray-900 border border-gray-200 rounded-lg px-3 py-2 bg-white outline-none focus:border-[#1450a0] focus:ring-2 focus:ring-[#1450a0/10] transition"
                  value={form.date}
            ...
            <div className="grid grid-cols-2 gap-3 mt-3">
              ...
              <Field label="키워드">
                <Input value={form.keyword} onChange={v => setField('keyword', v)} placeholder="ex. 동춘동정형외과" />
              </Field>
            </div>
          </Section>
              ...
              {form.doctors.map((d, i) => (
                <div key={i} className="flex items-center gap-2 relative">
                  ...
                  <div className="flex-1 min-w-0 relative">
                    ...
                    />
                    {activeDoctorIdx === i && suggestions.length > 0 && (
      ...
      <div className="px-6 py-4 bg-white border-t border-gray-200 flex gap-2">
        ...
        <button
          onClick={async () => { setSaving(true); await handleSaveAndEdit(form); setSaving(false); }}
          disabled={saving}
          className="flex-1 py-2.5 text-sm font-medium rounded-lg bg-[#1450a0] text-white hover:bg-[#1045a0] disabled:opacity-40 transition-colors"
        >
          {saving ? '저장 중...' : '저장 후 편집기로 →'}
        </button>
      </div>
    </div>
  );
